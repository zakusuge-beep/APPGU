...
if menu == "🏠 หน้าแรก & Dashboard":
    ...
    if not df.empty:
        ...
        with col_m1:
            # --- 2. ระบบคำนวณแคลอรี่ (Calorie Calculator) ---
            st.subheader("🍽️ คำนวณปริมาณอาหารที่แนะนำ")
            # สูตร RER = 70 * (น้ำหนัก)^0.75 (แบบย่อคือ (Weight * 30) + 70 สำหรับสัตว์ 2-45 kg)
            rer = round((latest["น้ำหนัก"] * 30) + 70)
            st.info(f"พลังงานพื้นฐานที่ต้องการ (RER): **{rer} kcal/วัน**")
            
            fact = 1.6 # ตัวคูณเฉลี่ยสำหรับสัตว์ที่ทำหมันแล้ว/กิจกรรมปกติ
        ...
        with col_m2:
            # --- 3. ระบบตารางยาและเตือนความจำ (Medication Schedule) ---
